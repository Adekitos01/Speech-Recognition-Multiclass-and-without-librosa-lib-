#audio speech recognition without librosa library using spectogram
import os
import pathlib
...
# Verify that the dataset exist in that path 
DATASET_PATH ='C:/Users/Adekitos/Desktop/audio-dataset'
data_dir = pathlib.Path(DATASET_PATH)
if not data_dir.exists():
    print("The path does not exist or there is not dataset there")

# get the subdirectories as commands
...
#Split filenames into training, validation and test sets using a 80 (maybe more):10:10 ratio, respectively:
train_files = filenames[:84665]
val_files = filenames[84665:84665 + 10582]
test_files = filenames[-10582:]
...
#The mini Speech Commands dataset only contains mono recordings.
...
#Split the file paths into tf.RaggedTensors
...
#Create a tf.data.Dataset with Dataset.from_tensor_slices and Dataset.map,
#using get_waveform_and_label defined earlier.
...
#Now, define a function that transforms the waveform
#dataset into spectrograms and their corresponding labels as integer IDs:
...
#norm_layer = layers.Normalization(): this did not work
...
model = models.Sequential([
    ...
    #layers.Resizing(32, 32), this does not work again its old
...
model.save('audiowithoutlibrosamodel.h5');#saved model



#Run model on the test set and check the model's performance
test_audio = []
test_labels = []
